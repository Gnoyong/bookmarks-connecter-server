...
const initialize = async (callback) => {
  ...
  db.serialize(() => {
    ...
    const stmt = db.prepare(
      "INSERT INTO bookmarks (chrome_id, parent_id, name, url, type, user_id, date_added, hash, date_modified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
    );
...
bookmarkRouter.get("/view", (req, res) => {
  db = dbFactory.get();
  db.all("SELECT * FROM bookmarks WHERE deleted = 0", [], (err, rows) => {
    res.send(httpResponse.success(rows));
  });
});
...
bookmarkRouter.post("/delete/*", (req, res) => {
  db = dbFactory.get();
  db.run(
    `UPDATE bookmarks SET deleted = 1, date_modified = ${Date.now()} WHERE chrome_id = ? and user_id = 1`,
    req.params[0],
    function (err) {
      if (err) {
...
bookmarkRouter.post("/", (req, res, next) => {
  ...
  const stmt = db.prepare(
    "INSERT INTO bookmarks (chrome_id, parent_id, name, url, type, user_id, date_added, date_modified) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
  );
  stmt.run(
    node.id,
    node.parent_id,
